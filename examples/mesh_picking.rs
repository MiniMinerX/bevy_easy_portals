...
use bevy::{
    color::palettes::tailwind::{AMBER_400, VIOLET_400},
...
fn setup(
    ...
    let idle_material = materials.add(Color::from(AMBER_400));
    ...
    commands
        .spawn((
            ...
            Transform::from_xyz(1.5, 0.0, 0.0).with_rotation(Quat::from_rotation_z(-FRAC_PI_4)),
        ))
        .observe(rotate_on_drag)
        .observe(update_material_on::<Pointer<DragStart>>(drag_material))
        .observe(update_material_on::<Pointer<DragEnd>>(idle_material));

    let target = commands.spawn(Transform::from_xyz(1.5, 0.0, 2.0)).id();

    let rectangle = Rectangle::from_size(Vec2::splat(2.5));
    let portal_transform =
        Transform::from_xyz(-1.5, 0.0, -1.0).with_rotation(Quat::from_rotation_y(-FRAC_PI_4));
    ...
    commands
        ...
        .with_children(|parent| {
            parent
                ...
                .observe(update_material_on::<Pointer<Out>>(idle_material));
        });
}
...
fn rotate(
    mut positive: Local<bool>,
    mut query: Query<&mut Transform, With<Portal>>,
    time: Res<Time>,
) {
    for mut transform in &mut query {
        let (y, _x, _z) = transform.rotation.to_euler(EulerRot::YXZ);
        ...
        transform.rotate_y(sign * time.delta_secs() / 16.0);
    }
}
